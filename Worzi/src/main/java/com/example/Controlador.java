...
//	private Usuario usuarioActual;
	...
	public String paginaInicial(Model model, HttpSession sesion, HttpServletRequest request) {
		// Es imprescindible buscarlo en el repositorio para comprobar que existe y
	...
	public String userProfileView(Model model, HttpSession sesion) {
		 ...
		 * if(opt.isPresent()) { Usuario user = opt.get(); username =
		 * user.getNombreUsuario(); }
		 * 
		 * model.addAttribute("usuario", username);
		 * 
		 * // prueba //List<Usuario> usuarioActual = usuarioRepository.findAll();
		...
		// cargar los datos actualizados
		/*
		 * Optional<Usuario> opt = usuarioRepository.findById(userID); Usuario usu =
		 * opt.get();
		 * 
		 * 
	...
	public String addTablero(Model model, @RequestParam String nombre,
		...
		/*
		 * model.addAttribute("nombre", nombre); model.addAttribute("fecha", fechaFin);
		 * model.addAttribute("descripcion", descripcion);
		 * 
		 * Lista lista = new Lista("Lista "+numeroLista); Tarjeta t = new
		 * Tarjeta(nombre, fechaFin, descripcion);
		 * 
		 * model.addAttribute("tarjeta", t); lista.addTarjeta(t);
		 * 
	...
	public String addLista(Model model, @RequestParam String nombre, @RequestParam String nombreTablero,
		 ...
		 * opt.get();
		 * 
		 * // Se crea la lista con ese nombre y se añade al tablero del usuario Lista
		 * list = new Lista(nombre); listaRepository.save(list);
		 * usu.getTablero(0).addLista(list); usuarioRepository.save(usu);
		 * 
		 * model.addAttribute("usu", usu); model.addAttribute("tableros",
		 * usu.getTableros());
		 */
    ...
    public String tarjetaDELETE(Model model, @PathVariable long id, HttpSession sesion) {
        ...
        Tarjeta t = tarjetaRepository.findById(id).get();
        ...
        //Tablero tab = encontrarTablero(usuarioActual, l.getId());
        //tableroRepository.save(tab);

        Usuario u = usuarioRepository.findById(usuarioActual.getId()).get();
        model.addAttribute("usu", u);
        model.addAttribute("nombreUsuario", u.getNombreUsuario());
        //model.addAttribute("tableros", servicioTableros.getTableros());
        model.addAttribute("tableros", usuarioActual.getTableros());
	...
	public String modTarjeta(Model model, @RequestParam String nombre, @RequestParam(defaultValue = "") String fechaFin,
			...
			@RequestParam String listTarjetas, HttpSession sesion) {
		/*
		 * Optional<Usuario> opt = usuarioRepository.findById(userID); Usuario usu =
		 * opt.get();
		 * 
	...
	public String addTarjeta(Model model, @RequestParam String nombre, @RequestParam(defaultValue = "") String fechaFin,
		...
		/*
		 * Optional<Usuario> optu = usuarioRepository.findById(userID); Usuario usu =
		 * optu.get();
		 * 
		 * Optional<Lista> opt = listaRepository.findByNombre(listaAsociada); Lista
		 * list; if (opt.isPresent()) { // Si existe, se usa esa lista. list =
		 * opt.get(); } else { // Si no existe, se crea la lista list = new
		 * Lista(listaAsociada); listaRepository.save(list);
		 * usu.getTablero(0).addLista(list); } Tarjeta tarjeta = new
		 * Tarjeta(nombre,fechaFin,descripcion, list); list.addTarjeta(tarjeta);
		 * tarjetaRepository.save(tarjeta); listaRepository.save(list);
		 * 
		 * model.addAttribute("usu", usu); model.addAttribute("tableros",
		 * usu.getTableros());
		 */
		...
		servicioListas.guardarLista(list);
		/*
		 * servicioTableros.getTableroById(tab.getId()); tab.addLista(list);
		 * servicioListas.guardarLista(list); servicioTableros.guardarTablero(tab);
		 */
	...
	/*
	 * @GetMapping("/nuevoUsuario") public String registrarse1(Model model) {
	 * 
	 * return "registrarse"; }
	 */

	/*
	 * @PostMapping("/nuevoUsuario") public String registrarse(Model
	 * model, @RequestParam String nombreUsuario,
	 * 
	 * @RequestParam String email, @RequestParam String contrasenya) {
	 * 
	 * Usuario usuario = new Usuario(nombreUsuario, email, contrasenya);
	 * usuarioRepository.save(usuario); // usuarioActual = usuario; userID =
	 * usuario.getId();
	 * 
	 * model.addAttribute("usu", usuario);
	 * 
	 * Tablero t = new Tablero("Tablero de "+usuario.getNombreUsuario(), usuario);
	 * usuario.addTablero(t); tableroRepository.save(t);
	 * model.addAttribute("tableros", usuario.getTableros()); return "main"; }
	 */
	/*
	 * @GetMapping("/iniciarSesion") public String iniciarSesion() { return
	 * "iniciarSesion"; }
	 */
	/*
	 * @PostMapping("/usuarioAcceso") public String iniciarSesion(Model model,
	 * Usuario usuario, HttpSession sesion) { Optional<Usuario> opt =
	 * usuarioRepository.findByNombreUsuarioAndContrasenyaAndEmail(usuario.
	 * getNombreUsuario(), usuario.getContrasenya(), usuario.getEmail());
	 * if(opt.isPresent()) { Usuario usu = opt.get(); // usuarioActual = usu; userID
	 * = usu.getId(); model.addAttribute("usu", usu); model.addAttribute("tableros",
	 * usu.getTableros()); sesion.setAttribute("usuarioActual", usuario); return
	 * "main"; } else { return "usuarioNoExiste"; }
	 * 
	 * }
	 */

	/*
	 * @GetMapping("/perfil") public String userProfileView(Model model, HttpSession
	 * sesion) {
	 */
	/*
	 * String username = "";
	 ...
	 * 
	 * 
	 * model.addAttribute("nombre", usu.getNombreUsuario()); return "miPerfil"; }
	 */
	/*
	 * @GetMapping("/crearTablero") public String verTarjeta(Model model) { return
	 * "GetTablero"; }
	 */
	/*
	 * @PostMapping("/Tablero") public String addTablero(Model model, @RequestParam
	 * String nombre,
	 * 
	 * @RequestParam(required=false, defaultValue="") String fechaFin
	 ...
	 * 
	 * usu.addTablero(tablero); tableroRepository.save(tablero);
	 * 
	 * return "main"; }
	 */
	...
	// GET PAGINA CREACION DE LISTAS
	/*
	 * @GetMapping("/crearLista") public String crearLista(Model model) { return
	 * "GetLista"; }
	 */

	/*
	 * @PostMapping("/Lista") public String addLista(Model model, @RequestParam
	 * String nombre) {
	 * 
	 * Optional<Usuario> opt = usuarioRepository.findById(userID); Usuario usu =
	 * opt.get();
	 ...
	 */
	/*
	 * @GetMapping("/crearTarjeta") public String crearTarjeta(Model model) {
	 * 
	 * return "GetTarjeta"; }
	 * 
	 * @GetMapping("/borrar/tarjeta/{id}") public String borrarTarjeta(Model
	 * model, @PathVariable long id) {
	 * 
	 * Optional<Tarjeta> optj = tarjetaRepository.findById(id); if(optj.isPresent())
	 * { Tarjeta tarj = optj.get(); Lista l = tarj.getListaAsociada();
	 * l.removeTarjeta(tarj); listaRepository.save(l);
	 * tarjetaRepository.delete(tarj); } Optional<Usuario> optu =
	 * usuarioRepository.findById(userID); Usuario usu = optu.get();
	 * model.addAttribute("usu", usu); model.addAttribute("tableros",
	 * usu.getTableros()); return "main"; }
	 * 
	 * 
	 * @PostMapping("/Tarjeta") public String addTarjeta(Model model, @RequestParam
	 * String nombre, @RequestParam(defaultValue="") String fechaFin,
	 * 
	 * @RequestParam(defaultValue="") String descripcion, @RequestParam String
	 * listaAsociada) {
	 * 
	 * Optional<Usuario> optu = usuarioRepository.findById(userID); Usuario usu =
	 * optu.get();
